...
const pinRuleName = document.querySelector('#pinRuleName');
...
function renderParameterRule() {
  pinRule.checked = parameterRule.enabled;
  pinRulePosition.value = parameterRule.position;
  pinRuleName.value = parameterRule.name;
  pinRulePosition.disabled = !parameterRule.enabled;
  pinRuleName.disabled = !parameterRule.enabled;
}

async function saveParameterRule() {
  const saved = await chrome.storage.local.get('gatewayAutofillParameterRuleByEnvironment');
  const rules = saved.gatewayAutofillParameterRuleByEnvironment || {};
  rules[environmentKey] = parameterRule;
  await chrome.storage.local.set({ gatewayAutofillParameterRuleByEnvironment: rules });
}

function updateParameterRule() {
  parameterRule = {
    enabled: pinRule.checked,
    position: pinRulePosition.value,
    name: pinRuleName.value
  };
  renderParameterRule();
  saveParameterRule();
}

function getParameterRule() {
  const position = Number(pinRulePosition.value);
  const name = pinRuleName.value.trim();
  if (!Number.isInteger(position) || position < 1) return { error: '参数替换规则的位置应为大于 0 的整数。' };
  if (!name) return { error: '请填写参数替换规则名称。' };
...
async function initialise() {
  ...
  const saved = await chrome.storage.local.get([
    'gatewayAutofillConfig', 'gatewayAutofillConfigByEnvironment', 'gatewayAutofillEnvironmentSchema', 'gatewayAutofillDisplayMode', 'gatewayAutofillConfigCollapsed', 'gatewayAutofillParameterRuleByEnvironment'
  ]);
  ...
  parameterRule = { enabled: false, position: '1', name: 'pin', ...((saved.gatewayAutofillParameterRuleByEnvironment || {})[environmentKey] || {}) };
...
pinRule.addEventListener('change', updateParameterRule);
...
button.addEventListener('click', async () => {
  ...
  const parameterRule = pinRule.checked && serviceType() === 'JSF服务' ? getParameterRule() : null;
  if (parameterRule?.error) return show(parameterRule.error, true);
  button.disabled = true;
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.id || !/^https?:\/\/(uat-)?gateway\.jdl\.com\//.test(tab.url || '')) throw new Error('请先切换到预发或线上网关的 API 新增页面。');
    const result = await chrome.tabs.sendMessage(tab.id, { type: 'gateway-autofill', text: template.value, addPinRule: Boolean(parameterRule), parameterRule, serviceType: serviceType(), config });
